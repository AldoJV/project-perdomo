...
CREATE TABLE IF NOT EXISTS `games`.`articulo` (
  ...
  `i_formato` VARCHAR(1) NOT NULL,
  `i_clasificacion` VARCHAR(3) NULL,
  `i_genero` VARCHAR(3) NULL,
  ...
  INDEX `fk_articulo_clas_idx` (`i_clasificacion` ASC),
  INDEX `fk_articulo_gen_idx` (`i_genero` ASC),
  ...
  CONSTRAINT `fk_articulo_formato`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_articulo_clas`
    FOREIGN KEY (`i_clasificacion`)
    REFERENCES `games`.`clasificacion` (`i_clasificacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_articulo_gen`
    FOREIGN KEY (`i_genero`)
    REFERENCES `games`.`genero` (`i_genero`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `games`.`cliente` (
  ...
  UNIQUE INDEX `n_username_UNIQUE` (`n_username` ASC),
...
CREATE TABLE IF NOT EXISTS `games`.`review` (
  `i_review` INT NOT NULL AUTO_INCREMENT,
  `i_articulo` INT(10) NOT NULL,
  `i_cliente` INT(10) NOT NULL,
  `q_puntuacion` INT(1) NOT NULL,
  `e_review` VARCHAR(500) NOT NULL,
  PRIMARY KEY (`i_review`),
  INDEX `fk_review_1_idx` (`i_articulo` ASC),
  INDEX `fk_review_2_idx` (`i_cliente` ASC),
